...
var Block = function(type, number) {
    ...
    if (type == 0) {
        ...
        var geometry = new THREE.BoxGeometry(_config.BLOCK.radius * 2, this.height, _config.BLOCK.radius * 2);
        this.map = new THREE.TextureLoader().load('res/bag.png');
        var material = new THREE.MeshLambertMaterial({
            map : this.map
        });
        ...
        this.record.add(new THREE.Mesh(new THREE.CylinderGeometry(_config.BLOCK.radius * 0.9,
        ...
        this.min = 0.8;
        this.height = _config.BLOCK.height / 21 * 4;
        var geometry = new THREE.CylinderGeometry(_config.BLOCK.radius * 0.7, _config.BLOCK.radius * 0.8, this.height,
                50);
        this.geometry = geometry;
        ...
        var deskGeometry = new THREE.BoxGeometry(_config.BLOCK.radius * 2, this.height,
        ...
        this.hitObj = new THREE.Mesh(geometry, material);
        this.shadow = new THREE.Mesh(new THREE.PlaneGeometry(this.shadowWidth, this.shadowWidth),
        ...
        this.height = _config.BLOCK.height / 21 * 6;
        var geometry = new THREE.BoxGeometry(_config.BLOCK.radius * 2 / 38 * 45, this.height,
        ...
        var geometry = new THREE.CylinderGeometry(_config.BLOCK.radius * 2 / 38 * 25,
                _config.BLOCK.radius * 2 / 38 * 25, this.height, 50);
        this.geometry = geometry;
        ...
        var geometry = new THREE.CylinderGeometry(_config.BLOCK.radius * 2 / 38 * 15,
                _config.BLOCK.radius * 2 / 38 * 15, this.height, 50);
        this.geometry = geometry;
        ...
        this.min = 0.8;
        this.height = _config.BLOCK.height / 21 * 4;
        var geometry = new THREE.CylinderGeometry(_config.BLOCK.radius * 0.4, _config.BLOCK.radius * 0.4, this.height,
                50);
        this.geometry = geometry;
...
Block.openDoor = function() {
    _animation.customAnimation.to(this.door.position, 1, {
        z : -4.5
    });
    _animation.customAnimation.to(this.secondDoor.position, 1, {
        z : -0.5
    });
};
    ...
    for (var i = 0; i < 2; ++i) {
        setTimeout(function(icon) {
            return function() {
                ...
                // customAnimation.to(this.icons[i].position, 3, { y:
                // 15, delay: i * 1 });
                // customAnimation.to(this.icons[i].position, 3, { z: 10
                // * (1 - 2 * Math.random()), delay: i * 1 });
